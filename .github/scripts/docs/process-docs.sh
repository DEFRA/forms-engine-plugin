...
find "./schemas" -type f -name "*.md" | while read file; do
...
if [ -f "./schemas/index.md" ] && [ -f "./schemas/README.md" ]; then
  echo "🗑️ Removing redundant schemas/README.md since index.md exists..."
  rm "./schemas/README.md"
...
EOF
  ...
  sed -n '/^---$/,/^---$/!p' "./schemas/index.md" >> "./temp_schema_ref.md"
  
  # Replace the SCHEMA_REFERENCE.md file
  mv "./temp_schema_ref.md" "./SCHEMA_REFERENCE.md"
  
  # Convert any relative links to absolute links and remove .md
  sed "${SED_INPLACE[@]}" -E 's|\* \[([^]]+)\]\(([^/][^)]+)\.md\)|\* \[\1\]\(\/schemas\/\2\)|g' "./SCHEMA_REFERENCE.md"
  sed "${SED_INPLACE[@]}" -E 's|\* \[([^]]+)\]\(([^/][^)]+)\)|\* \[\1\]\(\/schemas\/\2\)|g' "./SCHEMA_REFERENCE.md"
...
if [ -f "./schemas/index.md" ]; then
  ...
  sed "${SED_INPLACE[@]}" 's|permalink: /schemas/|permalink: /schemas/index/|g' "./schemas/index.md"
fi

# Fix relative links within the same directory to use absolute paths
echo "🔧 Fixing relative links within the same directory..."
find "./features" -type f -name "*.md" | while read file; do
  dir=$(dirname "$file")
  base_dir=${dir#./}  # Remove ./ prefix
...
for liquid_file in "./features/configuration-based/PAGE_EVENTS.md" "./features/configuration-based/PAGE_TEMPLATES.md" "./features/code-based/PAGE_VIEWS.md"; do
