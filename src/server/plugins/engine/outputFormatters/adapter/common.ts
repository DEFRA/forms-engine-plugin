...
import { type FormAdapterSubmissionSchemaVersion } from '~/src/server/plugins/engine/types/enums.js'
import {
...
/**
 * The payload every adapter schema version shares.
 *
 * Later versions add to this rather than reshape it, so each version's
 * formatter is only the difference between it and the one before.
 */
